...
class NewStructureRepositoryReader:
    ...
    def load_normalized_documents(
        ...
        if api_info_path.exists():
            ...
            documents.append(self._normalize_api_basic_info(api_info))
    ...
    def _normalize_api_basic_info(self, data: Dict[str, Any]) -> Dict[str, Any]:
        new_spec = {"api": data.get("spec", {})}
        # Remove fields that are not used by the script
        for field in ["id", "creationDate", "lastUpdate"]:
