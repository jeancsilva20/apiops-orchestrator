...
class JsonToYamlService:
    ...
    def _create_interceptors_part(self) -> Dict[str, Any]:
        self.logger.debug(f"Creating interceptors part ({len(self.json_full_object.interceptors)} total)")
        try:
            interceptors_list = [self._prepare_interceptor(i) for i in self.json_full_object.interceptors]
            self.logger.debug(f"All {len(interceptors_list)} interceptor(s) prepared successfully")
            ...
            self.logger.error(f"Error creating interceptors part: {str(e)}", exc_info=True)
            raise

    def _create_resources_and_ops_parts(self):
        self.logger.debug(f"Creating resources and operations parts for {len(self.json_full_object.resources)} resource(s)")
        resources_output_list = []
        operation_files = []

        try:
            for idx, resource in enumerate(self.json_full_object.resources, 1):
                self.logger.debug(f"Processing resource {idx}/{len(self.json_full_object.resources)}: {resource.name}")
                
                # Makes sure operation will be searched
                ops_list = getattr(resource, 'operations', [])
                self.logger.debug(f"Resource '{resource.name}' contains {len(ops_list)} operation(s)")
            ...
            self.logger.info(f"Resource and operations creation completed: {len(operation_files)} operation(s)")
            return resources_output_list, operation_files
        except Exception as e:
            self.logger.error(f"Error creating resources and operations: {str(e)}", exc_info=True)
            raise
    ...
    def _create_operation_files_and_refs(self, operations_list: List[Any]) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        """
        self.logger.debug(f"Creating operation files and references ({len(operations_list)} operation(s))")
        ops_refs = []
        operation_files = []

        try:
            for idx, op in enumerate(operations_list, 1):
                self.logger.debug(f"Processing operation {idx}/{len(operations_list)}: {op.method} {op.path}")
                ...
                if 'interceptors' in op_data:
                    ...
                    self.logger.debug(f"Processed {num_interceptors} interceptor(s) in operation {op.method} {op.path}")
                ...
                self.logger.debug(f"Operation {op.method} {op.path} processed successfully")

            self.logger.debug(f"Operation creation completed: {len(ops_refs)} reference(s), {len(operation_files)} file(s)")
            return ops_refs, operation_files
        except Exception as e:
            self.logger.error(f"Error creating operation files and references", exc_info=e)
            raise
    ...
    def _create_resource_entry(self, resource: Any, ops_refs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        self.logger.debug(f"Creating resource entry: {resource.name} with {len(ops_refs)} reference(s)")
        try:
            entry = {
            ...
            self.logger.error(f"Error creating resource entry '{resource.name}': {str(e)}", exc_info=True)
            raise
        ...
        try:
            ...
            if isinstance(content, str):
                try:
                    ...
                    self.logger.debug(f"Failed to convert interceptor JSON, keeping as string: {str(e)}")
                    pass
            ...
            if isinstance(i_dict.get('content'), str) and i_dict['content'].isdigit():
                ...
                self.logger.debug(f"Numeric content converted to integer: {i_dict['content']}")
