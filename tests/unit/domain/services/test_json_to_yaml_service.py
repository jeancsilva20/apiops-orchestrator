...
from apiops_orchestrator.application.services.file_importer_service import FileImporterService
from apiops_orchestrator.adapters.inbound.files_importer.file_loader_strategy import FILE_LOADER_STRATEGIES
from apiops_orchestrator.adapters.inbound.files_importer.local_file_importer_adapter import LocalFileLoaderAdapter
...
class MockApiPartialInfo:
    ...
    description: str
    basePath: str
    apiResponsible: MockApiResponsible = field(default_factory=lambda: MockApiResponsible(username="testuser", groupName="Test Group"))
...
def schema_validator(settings) -> SchemaValidator:
    ...
    schema_folder = settings.PROJECT_ROOT / "src" / "apiops_orchestrator" / "domain" / "schemas"
    return SchemaValidator(file_importer_service=file_importer_service, schema_folder=schema_folder)
            ...
            MockResource(
                ...
                operations=[
                    MockOperation(
                        ...
                        interceptors=[MockInterceptor(name="Op-Specific-Interceptor", position=2)]
                    ),
                    MockOperation(path="/users/{id}", method="POST", description="Create a user")
                ]
            )
        ]
    )
...
class TestJsonToYamlService:
    def test_build_yaml_parts_and_validate_schemas(
        ...
        schema_map = {
            ...
            JsonKind.INTERCEPTORS.value: "test-interceptors.schema.json"
        }
