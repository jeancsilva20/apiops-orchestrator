...
- **Composition root lazy (ADR 0006)**: construção de `Settings()` e demais dependências migra para builders invocados sob demanda — metadados da CLI (`sen`, `sen --help`, `sen list --help`) respondem instantaneamente com ou sem `.env` (D1-a).
...
- **Fluxo legacy extraído (D3)**: o fluxo `python main.py` pelado (validação de estrutura + schema + conversão `ApiFull`) é extraído para `scripts/generate_api_json.py`, fora do pacote, liberando o `main.py` para ser composition root exclusivo da CLI.
...
- `src/apiops_orchestrator/main.py` — reescrito para composition root lazy; fluxo legacy movido para `scripts/generate_api_json.py`.
